...
| 단계 | 방식 | 반응 속도 | 시스템 다운 시 |
|------|------|-----------|---------------|
| 1차 | KIS WebSocket 실시간 (H0STCNT0) | tick 즉시 | ❌ 동작 안 함 |
| 2차 | 거래소 지정가 사전 주문 | 즉시 (거래소) | ✅ 자동 체결 |
| 3차 | 90초 폴링 백업 | 최대 90초 | ❌ 동작 안 함 |

```
매수 체결
  ├→ [1차] WebSocket 구독 시작 (tick마다 손절선 비교)
  └→ [2차] 거래소 지정가 손절 주문 제출

트레일링 손절선 상향
  ├→ [1차] WebSocket 콜백의 비교 기준 자동 갱신 (DB 읽음)
  └→ [2차] 거래소 주문 취소 + 새 가격 재제출

손절선 돌파 감지
  ├→ [1차] _on_ws_price_tick() → 즉시 시장가 매도 + 구독 해제
  └→ [2차] 거래소 지정가 주문이 이미 취소됨 (이중 매도 방지)
```

**WebSocket 모듈:** `src/infra/kis_websocket.py`
- KISWebSocket 싱글턴, 자동 재연결, 구독 복원
- TR_ID: `H0STCNT0` (주식 체결), 인가키: `/oauth2/Approval`
- 장외 시간에는 메시지 수신 없음 → 폴링(3차)이 백업 역할 유지
...
차단 조건이 하나라도 충족되면 Claude `analyze()` 호출 없이 즉시 `[]` 반환.

| Gate | 조건 | 데이터 소스 |
|------|------|-------------|
| Gate 1 | 리스크 레벨 ≥ 4 | `risk_status` 테이블 |
| Gate 2 | 글로벌 시황 outlook == `negative` | `global_condition` 테이블 |
| Gate 3 | 국내 시황 점수 < -0.3 | `market_condition` 테이블 (이미 조회된 값 재사용) |

> 리스크팀이 아직 미기동 상태(DB 레코드 없음)인 경우 Gate 1은 무시하고 진행한다.
