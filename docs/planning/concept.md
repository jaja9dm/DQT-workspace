...
매 1~2분마다 보유 전 종목 스캔:
  ...
  손절 (즉시 시장가 전량 청산):
    - 기본: 평균 단가 대비 -5% 이하
    - 리스크 레벨 2 이상: -3% 이하로 강화
    - 리스크 레벨 4 이상: -1% 이하로 강화

  분할 익절 (지정가):
    - 1차: +5% 도달 → 보유 수량 1/3 매도
    - 2차: +10% 도달 → 보유 수량 1/3 매도
    - 잔여: 타임컷 또는 손절 기준 도달까지 보유

  타임컷 (장 마감 30분 전 자동 매도 검토):
    - 보유 5 영업일 초과 종목
    - 수익 중이면 1차 익절 처리
    - 손실 중이면 손절 처리
...
| 알림 발송 | **카카오톡 API / Slack Webhook** | 리포트 발송 채널 |
...
| 에러 유형 | 처리 방식 | 알림 |
|----------|----------|------|
| 네트워크 오류 | 3회 재시도 (5s → 15s → 30s) | 3회 실패 시 Slack |
| KIS API 오류 | 주문 취소 + 로그 저장 | 즉시 Slack |
| Claude API 오류 | 해당 팀 skip, 이전 결과 유지 | 실패율 30%↑ 시 Slack |
| DB 쓰기 오류 | 로컬 CSV 백업 | 즉시 Slack |
| 토큰 만료 | 자동 재발급 시도 | 재발급 실패 시 거래 중지 |
| 팀 프로세스 크래시 | 자동 재시작 (최대 3회) | 즉시 Slack |
| 예상 외 예외 | 전체 프로세스 중지 + 스택트레이스 저장 | 즉시 Slack |
...
| 수집 실패 처리 | 3회 재시도 (backoff: 5s → 15s → 30s) → 실패 시 스킵 + Slack 알림 |
...
| 에러 유형 | 처리 방식 | 알림 |
|----------|----------|------|
| 네트워크 오류 | 3회 재시도 (exponential backoff) | 3회 모두 실패 시 Slack |
| Claude API 오류 | 해당 종목 `skip` 처리 | 실패율 50% 초과 시 전체 중지 |
| KIS API 주문 오류 | 주문 취소 + 로그 저장 | 즉시 Slack |
| DB 쓰기 오류 | 로컬 CSV 백업 파일에 임시 저장 | 즉시 Slack |
| 토큰 만료 | 자동 재발급 시도 | 재발급 실패 시 거래 중지 |
| 예상 외 예외 | 전체 프로세스 중지 + 스택트레이스 저장 | 즉시 Slack |
