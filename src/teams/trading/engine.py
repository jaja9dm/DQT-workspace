...
### 최우선: 갭업 돌파 (gap_up_breakout) 신호
전일 대비 +8% 이상 갭업한 종목 중 OBV 양수이면 즉시 매수 권고.
- 목표 수익률: 갭업 비율의 30~50% (갭 +15%면 목표 +4~7%)
- 손절: 1.5%로 타이트 (갭업 후 되돌림 시 빠른 컷)
- RSI가 80↑이어도 OBV 양수 + 거래량 폭발이면 buy: true (stop_pct는 1.5% 고정)
- 오전(11:00 이전)에만 선정됨 — 오후 뒤늦은 추격은 이미 시스템이 차단
...
**중요: 손익비(target_pct / stop_pct) ≥ 2.0 이상이 되도록 설정하세요.**
기대 수익이 리스크의 2배 이상인 경우에만 진입 가치가 있습니다.
...
class TradingEngine:
    ...
    def run_once(self) -> list[dict]:
        ...
        for c in candidates:
            ...
            # 완전 차단 조건 (갭업 돌파는 일부 예외 적용)
            fails = []
            if vol_ratio < _min_vol:
                # 갭업 돌파 종목은 거래량 기준 1.2x로 완화 (갭업 자체가 강한 수급 신호)
                _vol_min_eff = 1.2 if is_gap_up else _min_vol
            ...
            if rsi > _max_rsi_hard:
                # 갭업 돌파 + OBV 양수이면 RSI 90까지 허용 (rsi_hot 플래그로 50% 포지션)
            ...
            if obv_slope < 0 and rsi > 70:
                # 갭업 돌파 종목은 OBV 역행 차단 완화 (장 초반 일시 지표 왜곡 가능)
                if not is_gap_up:
                    fails.append(f"OBV 역행+고RSI {rsi:.0f} (수급 없는 상승)")
            # 당일 고가권 추격 매수 차단 — 갭업 돌파 + OBV 양수이면 예외 ("추격"이 아닌 "돌파")
            if price_chg >= 3.0 and day_range_pos >= 0.90 and obv_slope <= 0:
                if not is_gap_up:
                    fails.append(
                        f"고가권 추격 차단 (등락 {price_chg:+.1f}% + 범위위치 {day_range_pos:.2f} + OBV↓)"
                    )
            # 갭업 돌파 종목은 오전(11:00 이전)에만 진입 — 오후 뒤늦은 추격 방지
            if is_gap_up and _hm >= 1100:
                fails.append(f"갭업 돌파 오전 진입 한정 (현재 {now.strftime('%H:%M')} > 11:00)")
