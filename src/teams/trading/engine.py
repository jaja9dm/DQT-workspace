    ...
    def run_once(self) -> list[dict]:
        ...
        for c in candidates:
            ...
            if vol_ratio < _min_vol:
                # 갭업/눌림/강세편승은 거래량 기준 1.2x로 완화
                _vol_min_eff = 1.2 if (is_gap_up or is_pullback or is_mkt_mom) else _min_vol
                if vol_ratio < _vol_min_eff:
                    fails.append(f"거래량비 {vol_ratio:.1f}x < {_vol_min_eff:.1f}x")
            # 눌림목 반등: 오늘 하락 종목이므로 등락률 ≤ 0 차단 면제
            if price_chg <= 0 and not is_pullback:
            ...
            # 눌림목 반등: RSI 낮은 것이 정상 — 과매도 하한선 면제
            if rsi < _min_rsi and not is_pullback:
