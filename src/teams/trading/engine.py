...
- VWAP위치: 현재가가 당일 VWAP(거래량가중평균가격) 대비 위치. +%= VWAP 위(기관 지지), -= VWAP 아래(수급 이탈)
...
## VWAP 해석 지침
- VWAP위치 +1%↑ = 기관·외국인 평균 단가 위 → 수급 주도 종목, 진입 신뢰도 높음
- VWAP위치 -0.5% ~ +1% = 중립 구간 — 다른 지표로 판단
- VWAP위치 -1%↓ = 수급 중심선 아래 (매도 우위) — momentum/gap_up은 보류 권고
- 단, pullback_rebound/opening_plunge_rebound는 VWAP 아래가 정상 (반등 기대 전략) — VWAP 차단 면제

## 슬롯별 매수 기준 (slot 필드)
- **leader 슬롯**: 시장 주도주 편승 — KOSPI 강세 + 섹터 수급 집중 종목. RSI 45~75, VWAP 위, 추세 종목
- **breakout 슬롯**: 돌파매매 — 신고가·거래량 급증·OBV 양수 확인 후 진입. RSI 50~82 OK. day_range_pos ≥ 0.5
- **pullback 슬롯**: 눌림목 매수 — 고점 대비 조정 후 반등. OBV 양수 + MACD buy_pre 필수. RSI ≤ 72. VWAP 아래 허용
- 슬롯 미배정(미배정): signal_type 기준으로 판단
...
class TradingEngine:
    ...
    def run_once(self) -> list[dict]:
        ...
        for c in candidates:
            ...
            # ── [진입-5] 슬롯별 특화 진입 조건 ──────────────────────────
            _sig_type_c = c.get("signal_type", "momentum")
            _drp_c2     = float(c.get("day_range_pos") or 0.5)
            _vol_c2     = float(c.get("volume_ratio") or 0.0)
            _obv_c2     = float(c.get("obv_slope") or 0.0)
            _rsi_c2     = float(c.get("rsi") or 50.0)

            # ── Leader: 추세 편승 — 눌림/급락반등 신호·극단적 바닥 제외 ──
            if is_leader_slot:
                if _sig_type_c in ("pullback_rebound", "opening_plunge_rebound"):
                    logger.info(
                        f"Gate 4.5 차단: {tk} leader 슬롯 — "
                        f"signal '{_sig_type_c}'은 pullback 슬롯 전용"
                    )
                    continue
                if _drp_c2 < 0.25:
                    logger.info(
                        f"Gate 4.5 차단: {tk} leader 슬롯 — "
                        f"day_range_pos {_drp_c2:.2f} < 0.25 (주도주 아닌 하락 종목)"
                    )
                    continue
                gated_candidates.append(c)
                continue

            # ── Breakout: 돌파 — 신고가+3배면 즉시, 그 외 위치·거래량·OBV 확인 ──
            if is_breakout_slot:
                _new_high_c = bool(c.get("at_new_high", 0))
                # 신고가 + 거래량 3배: 즉시 진입 (MACD 대기 면제)
                if _new_high_c and _vol_c2 >= 3.0:
                    logger.info(
                        f"Gate 4.5 통과: {tk} breakout 신고가+거래량{_vol_c2:.1f}x 즉시 진입"
                    )
                    gated_candidates.append(c)
                    continue
                # 일반 breakout: 돌파 위치 확인 (day_range_pos ≥ 0.5)
                if _drp_c2 < 0.5:
                    logger.info(
                        f"Gate 4.5 차단: {tk} breakout 슬롯 — "
                        f"day_range_pos {_drp_c2:.2f} < 0.5 (돌파 위치 아님)"
                    )
                    continue
                # 거래량 확인 (2배 이상)
                if _vol_c2 < 2.0:
                    logger.info(
                        f"Gate 4.5 차단: {tk} breakout 슬롯 — "
                        f"거래량비 {_vol_c2:.1f}x < 2.0 (돌파 거래량 불충분)"
                    )
                    continue
                # OBV 양수 필수 (기관 매수 확인)
                if _obv_c2 <= 0:
                    ...
                    continue
                # fall-through → 갭업·09:10 체크로 이어짐

            # ── Pullback: 눌림반등 — OBV·RSI·MACD buy_pre 하루 종일 필수 ──
            if is_pullback_slot:
                # 눌림 위치: day_range_pos ≤ 0.70 (장중눌림 태그 있으면 면제)
                if _drp_c2 > 0.70 and not c.get("_intraday_pullback"):
                    logger.info(
                        f"Gate 4.5 차단: {tk} pullback 슬롯 — "
                        f"day_range_pos {_drp_c2:.2f} > 0.70 (눌림 미발생)"
                    )
                    continue
                # OBV 양수 필수 (기관이 눌림에서 받아줘야 반등)
                if _obv_c2 <= 0:
                    logger.info(
                        f"Gate 4.5 차단: {tk} pullback 슬롯 — "
                        f"OBV 음수 (기관 지지 없음 — 낙폭 확대 위험)"
                    )
                    continue
                # RSI ≤ 72 (눌림 후 과열은 반등 완료, 추격 금지)
                if _rsi_c2 > 72:
                    logger.info(
                        f"Gate 4.5 차단: {tk} pullback 슬롯 — "
                        f"RSI {_rsi_c2:.0f} > 72 (눌림 후 과열 — 반등 소진)"
                    )
                    continue
                # MACD buy_pre 하루 종일 필수 (반전 신호 없으면 낙폭 확대)
                if macd_now != "buy_pre" and tk not in self._macd_reentry_ok:
                    logger.info(
                        f"Gate 4.5 차단: {tk} pullback 슬롯 — "
                        f"MACD {macd_now} (buy_pre 필수 — 반전 미확인)"
                    )
                    continue

            # ── 갭업 09:10 차단 + 09:30 전 MACD 대기 (pullback 제외) ──
            if not is_pullback_slot:
    ...
    def _ask_claude_batch(
        ...
        for item in items:
            ...
            if _atr > 0:
                ...
                _atr_tag = ""
            # VWAP 위치 (현재가 vs 당일 VWAP)
            _vwap_val, _vwap_cur = _get_vwap_position(ticker)
            if _vwap_val > 0 and _vwap_cur > 0:
                _vwap_diff = (_vwap_cur / _vwap_val - 1) * 100
                _vwap_tag = f"VWAP위치: {_vwap_diff:+.1f}%"
            else:
                _vwap_tag = "VWAP위치: 계산중"

            stock_lines.append(
                f"- 티커: {ticker} ({item.get('name', '')}) [슬롯: {slot}]\n"
                f"  신호: {_sig} | "
...
def _get_vwap_position(ticker: str) -> tuple[float, float]:
    """당일 VWAP + 현재가. 데이터 부족 시 (0.0, 0.0)."""
    from src.utils.vwap import get_vwap_and_price
    return get_vwap_and_price(ticker)
