...
## 운영 방침 (중요)
현재는 알고리즘 테스트 기간입니다. 로직 검증을 위해 최대한 많은 매매를 시도해야 합니다.
조건이 크게 나쁘지 않으면 즉시 매수(true)를 선택하세요.
리스크 레벨 4 이상이거나 글로벌 리스크 8 이상일 때만 관망을 선택하세요.
    ...
    if existing:
        ...
        execute(
            ...
            INSERT INTO trailing_stop
                (ticker, entry_price, trailing_floor, highest_price, ladder_bought, scale_in_count, dip_buy_count)
            VALUES (?, ?, ?, ?, 0, 0, 0)
            """,
