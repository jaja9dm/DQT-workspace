...
## 응답 형식 (STRICT JSON만 — 코드 펜스, 주석, 설명문, trailing comma 모두 금지)
{
  "adjustments": [
    {
      "param": "<param_name>",
...
- 배열의 마지막 원소 뒤에 콤마(,) 금지
- ```json 같은 마크다운 펜스 금지
- 조정 없으면 "adjustments": [], "code_changes_needed": [] 으로 빈 배열 반환"""
...
def _extract_json(raw: str) -> str:
    """Claude 응답에서 JSON 블록 추출 + 일반적 결함 보정.
    ...
    # 3. 줄 단위 주석 제거 (// 또는 # 시작 — 문자열 내부는 보존되도록 단순 라인 휴리스틱)
    cleaned_lines = []
    for line in text.splitlines():
        stripped = line.lstrip()
        if stripped.startswith("//") or stripped.startswith("#"):
            continue
        # 라인 끝 // 주석 제거 (문자열 내부 // 는 보호 — 따옴표 카운트)
        # 단순화를 위해 따옴표 밖에서만 잘라냄
        out = []
        in_str = False
        esc = False
...
위 데이터를 바탕으로 시스템 프롬프트의 규칙에 따라 JSON으로 응답하세요."""
    ...
    try:
        ...
        try:
            ...
            # 보정 후 재시도: 잘린 응답에 대비해 닫는 괄호 보강
            patched = cleaned
            # 미닫힘 배열/객체 추정 보강
            open_braces  = patched.count("{") - patched.count("}")
            open_brackets = patched.count("[") - patched.count("]")
            patched = patched + ("]" * max(0, open_brackets)) + ("}" * max(0, open_braces))
            try:
                result = json.loads(patched)
                logger.warning(
                    f"Claude 파라미터 튜닝 응답 자동 보정 성공 (원래 에러: {je.msg})"
                )
            except Exception:
                # 최종 실패 — 원본 일부 로그
                snippet = raw[:400].replace("\n", "\\n")
                logger.error(
                    f"Claude 파라미터 튜닝 응답 파싱 실패: {type(je).__name__}: {je} | raw[:400]={snippet}"
                )
                return {"adjustments": [], "code_changes_needed": []}
        ...
        logger.error(
            f"Claude 파라미터 튜닝 응답 파싱 실패: {type(e).__name__}: {e} | raw[:400]={snippet}"
        )
        return {"adjustments": [], "code_changes_needed": []}
