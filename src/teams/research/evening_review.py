...
분석 중 다음 케이스가 발견되면 needs_human_review=true로 명시하고 review_items에 사유·제안을 넣는다.
**진짜 사람이 코드를 수정해야 하는 경우만 true.** 단순 lessons 추가로 해결되면 false.

  - 평가 기준 자기 모순: 같은 결과가 적중/실패 양쪽으로 해석되는 케이스 (예: confidence=2 + 급등을 "실패"로 분류하는 것이 비논리적)
  - 명백한 코드 로직 오류 추정: 가격 추정값과 실제 값 30%+ 차이 등
  - 데이터 한계로 신뢰도 낮은 분석이 반복적으로 발생: 동일 결함이 3일 이상 누적
  - 동일 패턴의 lessons 3건 이상 연속 실패 → 룰 자체 재검토 필요
...
## 응답 형식 (STRICT JSON만 — 코드 펜스/주석/설명문/trailing comma 금지)
{
  ...
  "review_items": [
    {
      "reason": "<무엇이 모순/문제인지 1~2줄>",
      "suggestion": "<어떤 코드/룰을 어떻게 수정하면 좋을지 제안>"
    }
  ]
}
...
- needs_human_review: 기본 false. 위 4개 케이스 중 하나라도 해당하면 true.
- review_items: needs_human_review=true일 때만 작성. 0~3개."""
...
def _format_message(
    ...
    if orig_len > body_limit:
        msg = msg[:body_limit] + "\n...[중략]"
        logger.warning(
            f"[evening_review] 메시지 truncate — 본문 {orig_len}자 > 한도 {body_limit}자"
        )
    elif orig_len > body_limit - 200:
        logger.info(
            f"[evening_review] 메시지 길이 {orig_len}/{body_limit}자 (한도 근접)"
        )
    msg += legend
    return msg
...
def run_evening_review() -> dict:
    ...
    # 시나리오 A — Claude 자기 모순 / 검토 요청 감지 시 사람 개입 알림
    if review and review.get("needs_human_review") and send_human_alert is not None:
        review_items = review.get("review_items") or []
        for idx, item in enumerate(review_items[:3]):
            reason = (item.get("reason") or "").strip()
            suggestion = (item.get("suggestion") or "").strip()
            if not reason:
                continue
            try:
                title = f"검토 요청 — {reason[:60]}"
                body = (
                    f"사유:\n{reason}\n\n"
                    f"제안:\n{suggestion or '(Claude 제안 없음 — 사람이 직접 검토)'}\n\n"
                    f"📋 도출 시점: evening_review {today}\n"
                    f"📁 관련 파일: src/teams/research/evening_review.py"
                )
                send_human_alert(
                    ...
                    dedup_key=f"self_contradiction_{today}_{idx}",
                )
            except Exception as e:
                logger.warning(f"[evening_review] human_alert 발송 실패: {e}")
