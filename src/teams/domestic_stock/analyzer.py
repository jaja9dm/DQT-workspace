...
RSI 95 이하 + OBV 양수이면 선정 가능 — reason에 "RSI과열_포지션50%" 명시.
day_range 0.90↑이어도 갭업+OBV양수면 허용.
**갭업 종목은 테마 주도 상승 시 RSI 90대도 당일 추가 상승하는 패턴이 있으므로 RSI만으로 제외하지 말 것.**
...
## 거래량 최소 기준 (종목 품질 필터)
- 갭업/눌림목/오프닝급락 아닌 일반 모멘텀 종목: **거래량비 1.5x 미만이면 선정 불가**
