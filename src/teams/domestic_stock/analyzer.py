...
- **leader (주도주)**: 오늘 시장 자금이 가장 많이 몰리는 종목. ⭐주도주 태그 우선. 하루 종일 방향이 명확해야 함.
  거래대금 상위 + 외인 or 기관 순매수 + OBV↑ + RSI 82 이하. 모멘텀이 장 내내 유지될 종목.
  **[필수 조건 — 엄격 적용]** leader 슬롯은 후보 목록에서 거래대금이 가장 큰 종목으로 배정합니다. 거래대금 500억 미만 종목은 완전 배제. 기술지표(MACD·신고가·볼린저 등)가 다소 우세하더라도 거래대금이 작은 종목은 절대 leader가 될 수 없습니다. RSI 82 이하·OBV 상승 조건만 충족하면 거래대금 1위 종목을 반드시 leader로 선정하세요. 거래대금 = 기관·외인 자금 유입 = 하루 종일 방향성 유지의 핵심 지표입니다.

- **breakout (돌파매매)**: 저항선 돌파 or 갭업. 강한 수급이 단기 급등을 만드는 종목.
  갭업 +3%↑ or BB상단 돌파 + 거래량 3배↑ + MACD 양수. 조건 불충분하면 null 반환.

- **pullback (눌림목매매)**: 전일 강세 후 오늘 눌린 종목. 반등 타이밍 잡기.
  등락 -1%~-5% + OBV↑ + day_range 0.30 이하 저가권 + 시가 대비 반등 중. 조건 불충분하면 null 반환.
