...
- 외인순매수 양수(+) 종목: 외국인이 사고 있는 종목 — 기술적 신호와 함께라면 강한 복합 신호
- 외인순매수 음수(-) 종목: 외국인 이탈 중 — 기술적 신호가 강해도 신중하게 판단, 선정 기준 상향 적용
- 외인 순매수 규모가 클수록 (절대값 대비 거래량 비율) 수급 신뢰도 높음
...
- 외인 순매도 강할 때 (-1,000주 미만 or 외인비율 -1%↑): 기술적 신호만으로 선정 불가, 반드시 다른 복합 신호 필요
...
def _build_user_message(
    ...
    for s in candidates:
        ...
        frgn_str = f" | 외인순매수{s.frgn_net_buy:+,}주" if s.frgn_net_buy != 0 else ""
        lines.append(
            f"- {s.ticker}({s.name}): "
            f"등락{s.change_pct:+.1f}% | RSI {s.rsi:.0f} | "
            f"MACD히스토그램 {s.macd_hist:+.4f} | BB위치 {s.bb_position:.2f} | "
            f"MA20{'위' if s.above_ma20 else '아래'}{frgn_str} | {flag_str}"
        )
    stock_block = "\n".join(lines)
